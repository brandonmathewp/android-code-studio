...
package com.tom.rv2ide.services.ai

import com.tom.rv2ide.services.ai.Instructions.system_instructions
import com.tom.rv2ide.services.ai.preferences.getAgentName
...
/**
 * Service for interacting with DeepSeek AI API with full project context.
 *
...
class DeepSeekAIService : AIService {
  ...
  companion object {
    private val log = LoggerFactory.getLogger(DeepSeekAIService::class.java)
    private const val BASE_URL = "https://api.deepseek.com/chat/completions"
    private const val DEFAULT_MODEL_NAME = "deepseek-reasoner"
    private const val TIMEOUT_SECONDS = 60L

    private fun getAgentName(): String {
    ...
    private val MODEL_NAME: String
      get() =
          if (
              !getAgentName().isNullOrEmpty() &&
                  getAgentName().startsWith("Deepseek", ignoreCase = true)
          ) {
            getAgentName()
          } else {
            DEFAULT_MODEL_NAME
  ...
  fun initialize(apiKey: String) {
    try {
      this.apiKey = apiKey
      log.info("DeepSeek AI service initialized successfully with project awareness")
    } catch (e: Exception) {
      log.error("Failed to initialize DeepSeek AI service", e)
      throw e
    }
  }
        ...
        try {
          if (apiKey == null) {
            return@withContext Result.failure(
                IllegalStateException("DeepSeek AI service not initialized")
            )
          }
          ...
          val chatResponse =
              response.getOrNull()
                  ?: return@withContext Result.failure(Exception("Empty response from DeepSeek AI"))
  ...
  private suspend fun makeApiCall(requestBody: DeepSeekChatRequest): Result<DeepSeekChatResponse> {
    return try {
      val jsonBody = json.encodeToString(DeepSeekChatRequest.serializer(), requestBody)
      ...
      val chatResponse = json.decodeFromString(DeepSeekChatResponse.serializer(), responseBody)
      Result.success(chatResponse)
    } catch (e: Exception) {
      log.error("Error making API call to DeepSeek", e)
      Result.failure(e)
    }
  }
  ...
  /** Data classes for DeepSeek API communication */
  @Serializable
  ...
  @Serializable
  data class Choice(val index: Int, val message: ChatMessage, val finishReason: String? = null)

  @Serializable
  data class Usage(val promptTokens: Int, val completionTokens: Int, val totalTokens: Int)

  /** Data classes for AI Android development support (reused from Gemini) */
